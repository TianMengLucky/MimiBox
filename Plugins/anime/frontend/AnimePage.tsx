...
import type { BangumiCalendarItem, BangumiWeekday } from "./types";

/** 页面数据访问（index.tsx 里用插件上下文构造后传入） */
type AnimeApi = ReturnType<typeof createAnimeApi>;
...
export default function AnimePage({ api }: { api: AnimeApi }) {
            ...
            {active.items.length === 0 ? (
              ...
              >
                {active.items.map((item) => (
                  <AnimeCard key={item.id} item={item} api={api} />
                ))}
              </ul>
            )}
          </div>
...
function AnimeCard({ item, api }: { item: BangumiCalendarItem; api: AnimeApi }) {
  return (
    <li>
      <button
        type="button"
        ...
        onClick={() => void api.openItem(item.url)}
