...
/// 读取本地备注（解析失败降级为空数据）
pub(super) fn load_notes(host: &dyn HostApi) -> CommentNotes {
    store::load(host, FILE, LABEL).unwrap_or_default()
}

fn save_notes(host: &dyn HostApi, notes: &CommentNotes) -> Result<(), String> {
    store::save(host, FILE, LABEL, notes)
}
...
pub(super) fn set_edit(host: &dyn HostApi, rpid: i64, content: Option<String>) -> Result<(), String> {
    let mut notes = load_notes(host);
    match content {
        Some(text) => {
    ...
    save_notes(host, &notes)
}

/// 新增一条本地回复（挂在原评论下方），返回创建好的回复
pub(super) fn add_reply(host: &dyn HostApi, parent_rpid: i64, content: String) -> Result<LocalReply, String> {
    ...
    save_notes(host, &notes).map(|_| dto)
}

/// 修改一条本地回复的内容
pub(super) fn update_reply(host: &dyn HostApi, id: String, content: String) -> Result<(), String> {
    let content = content.trim().to_string();
    if content.is_empty() {
        return Err("回复内容不能为空".to_string());
    }
...
/// 删除一条本地回复
pub(super) fn remove_reply(host: &dyn HostApi, id: String) -> Result<(), String> {
    let mut notes = load_notes(host);
