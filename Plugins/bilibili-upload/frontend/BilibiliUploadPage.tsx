...
import type { BilibiliUploadApi, UploadParams } from "./api";
import type {
  SubmitOutcome,
  UploadProgress,
  UploadZoneMain,
...
type Copyright = UploadParams["copyright"];
...
export default function BilibiliUploadPage({ api }: { api: BilibiliUploadApi }) {
  // 表单状态
  ...
  useEffect(() => {
    ...
    void api
      .onProgress((next) => {
        if (submittingRef.current) setProgress(next);
      })
      .then((off) => {
        if (disposed) off();
        else unlisten = off;
  ...
  }, [api, videoPath, videoInfo, tid, title, desc, tags, copyright, source, coverUrl, noReprint, openElec, dynamic]);

  const cancel = useCallback(() => {
    void api.cancelUpload().catch(() => {});
  }, [api]);
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-3 text-center">
          ...
          <div className="flex items-center gap-2 pt-1">
            <Button
              ...
              onPress={() => {
                void api
                  .openOutcomeUrl(`https://www.bilibili.com/video/${outcome.bvid}`)
                  .catch(() => {});
              }}
            >
