...
import { BOBING_RANKS, evaluateDice } from "./rules";
import type { BobingRank } from "./rules";
import { createBobingStore } from "./store";
import type { BobingData } from "./types";

/** 页面数据读写（index.tsx 里用插件上下文构造后传入） */
type BobingStore = ReturnType<typeof createBobingStore>;
...
export default function BobingPage({ store }: { store: BobingStore }) {
  ...
  const update = useCallback((fn: (prev: BobingData) => BobingData) => {
    setData((prev) => {
      if (!prev) return prev;
      const next = fn(prev);
      store.save(next).catch((err) => console.error("保存博饼数据失败", err));
      return next;
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
