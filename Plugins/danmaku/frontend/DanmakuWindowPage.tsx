...
import type { DanmakuStatus } from "./types";
...
/**
 * 弹幕姬独立窗口（/w/danmaku，由插件后端 danmaku_open 打开）：无系统标题栏，
 * 自绘迷你标题栏（拖拽/最小化/关闭），主体为自动滚动的二次元风格弹幕列表。
 */
export default function DanmakuWindowPage({ api }: { api: DanmakuApi }) {
  const { messages, status } = useDanmakuFeed(api);
