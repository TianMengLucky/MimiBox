...
import { exportTierListImage } from "./export";
import { PoolPanel } from "./PoolPanel";
import { createTierListStore } from "./store";
import { TierRow } from "./TierRow";
import { DEFAULT_TIERS, MAX_ITEMS } from "./types";
import type {
  Tier,
  TierItem,
  TierListData,
...
/** 页面数据读写与导图命令（index.tsx 里用插件上下文构造后传入） */
type TierListStore = ReturnType<typeof createTierListStore>;
...
export default function TierListPage({ store }: { store: TierListStore }) {
  ...
  const handleExport = async () => {
    ...
    try {
      const path = await exportTierListImage(activeScheme, store.exportTierListImageFile);
