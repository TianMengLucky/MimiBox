...
/** 保存 PNG 文件的命令调用（由 store 提供，经插件上下文走网关写盘） */
type SaveImageFile = (name: string, dataUrl: string) => Promise<string>;
...
/** 按方案把梯队画成一张排名图 PNG，经 saveImageFile 交给插件后端保存到 exports/ 并返回文件路径 */
export async function exportTierListImage(
  scheme: TierListScheme,
  ...
  return saveImageFile(scheme.name || "排名表", canvas.toDataURL("image/png"));
}
