...
导入时宿主会校验插件清单与 ABI 版本（`abi: 1`），通过后复制到插件存放位置（默认为应用数据目录下的 `plugins/`，可在设置中自定义，见下）。**导入新插件即时生效**：宿主自动热加载并广播事件，主页/资料库卡片与功能页即时出现，无需重启；升级/替换同 id 插件与删除已加载插件受 Windows 动态库占用限制，仍需重启应用（设置页在需要时会提供一键重启按钮）。
