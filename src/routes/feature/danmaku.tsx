...
export const Route = createFileRoute("/feature/danmaku")({
  component: DanmakuFeatureRoute,
});
...
function DanmakuFeatureRoute() {
  const navigate = useNavigate();
  const { status } = useDanmakuFeed();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const list = await tauriInvoke<AccountEntry[]>("account_list");
        if (disposed) return;
  ...
  const start = useCallback(async () => {
    ...
    setActionError("");
    try {
      await tauriInvoke("danmaku_open");
